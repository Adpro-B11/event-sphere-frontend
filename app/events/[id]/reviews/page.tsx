...
export default function EventReviewsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (user?.email) {
          const existingUserReview = ReviewService.getUserReview(reviewsData, user.email)
  ...
  const handleSubmitReview = async (e: React.FormEvent) => {
    ...
    if (!ReviewService.userHasTicket(user.email, eventId)) {
  ...
  const handleDeleteReview = async () => {
    ...
    try {
      await ReviewService.deleteReview(eventId, userReview.id, '', user.username)
